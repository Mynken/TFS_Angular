...
export class UsersManagementComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        ...
        if (this.canManageUsers) {
            this.columns.push({ name: '', width: 130, cellTemplate: this.actionsTemplate, resizeable: false,
    ...
    loadData() {
        ...
        if (this.canViewRoles) {
            ...
            this.accountService.getUsers()
                .subscribe(
                    users => this.onDataLoadSuccessful(users, this.accountService.currentUser.roles.map(x => new Role(x))),
                    error => this.onDataLoadFailed(error));
        }
    }
